...
#define STANDARD_STACK_SIZE_WORDS 128
#define MINIMUM_STACK_SIZE_WORDS configMINIMAL_STACK_SIZE
#define INFINITE_WAIT portMAX_DELAY
#define NO_WAIT  (( TickType_t )0)

inline void Lock_Scheduler(void)
{
	vTaskSuspendAll();
}

inline BaseType_t Release_Scheduler(void)
{
...
public:
	...
	Task(TaskFunction_t code, char const * name = (char *)"TSK", uint16_t stack_size =
...
//! helper function to compute a suitable size (expressible as 2^n)
		...
		task(
				{
					...
					(uint16_t)fit_size( stacksize + sizeof(runner)/sizeof( portSTACK_TYPE)),
					(void *)&init_data,
...
{
public:
	timer(TickType_t period, TimerCallbackFunction_t callback,char *name = "TIMER")
	  : timer_ID(0)
	{
		timer_ID = xTimerCreate(name, period, 0, 0, callback);
		ASSERT(timer_ID != 0);
	}
	~timer( void)
	{
...
TaskHandle_t create_restricted_task( const TaskParameters_t *param);
